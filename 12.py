...
# 将base64编码的数据解码为原始数据
text_data['decoded_data'] = text_data[0].apply(lambda x: base64.b64decode(x).decode('utf-8'))
...
text_features = vectorizer.fit_transform(text_data['decoded_data'])
image_features = vectorizer.fit_transform(image_data['decoded_data'].astype(str))  # 转换为字符串以满足CountVectorizer的要求

# 准备训练数据
X = pd.concat([pd.DataFrame(text_features.toarray()), pd.DataFrame( image_features.toarray())])
y = pd.concat([pd.Series([0]*len(text_data)), pd.Series([1]*len(image_data))])
